    ...
    lines.forEach((line, idx) => {
      ...
      if (bullet || num) {
        ...
        if (isNumbered) {
          ...
          elements.push(
            <div key={listKey} className="flex items-start gap-3 ml-2 mb-2">
              <div className="min-w-[24px] h-6 bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold shadow-sm">
                {num![1]}
              </div>
              <span className="text-gray-800 leading-relaxed flex-1">{parsedContent}</span>
            </div>
          );
        } else {
          // Bullet with colored dot
      ...
      if (trimmed.startsWith('Sources:') || trimmed.startsWith('*Source:')) {
        ...
        elements.push(
          <div key={idx} className="mt-4 pt-3 border-t border-gray-200 text-xs text-gray-500 italic">
            <strong className="text-gray-600">Sources:</strong> {rest}
          </div>
        );
        inList = false;
        return;
      ...
      if (hasBold) {
        const parsed = parseInlineBold(trimmed);
        elements.push(
          <p key={idx} className="mt-1 text-gray-800 leading-relaxed">
            {parsed}
          </p>
        );
        inList = false;
      ...
      if (subItemMatch) {
        const subHeading = subItemMatch[1];
        const description = subItemMatch[2].trim();
        elements.push(
          <div key={idx} className="flex items-start gap-3 ml-2 mb-2">
            <div className="min-w-[8px] h-2 bg-blue-500 rounded-full mt-2 shadow-sm" />
